...
export class ArticleSearchComponent implements OnInit, AfterViewInit {
  isModalOpen = false;
  articleModal: Article | undefined;
  ...
  fetchResults(query: string): void {
    ...
    this.http
      .get<Article[]>(
        `https://control.nuevoloslagos.org/articles/search?search=${encodeURIComponent(
          query
        )}`
      )
      .subscribe({
  ...
  openModal(index: number): void {
    ...
    this.articleModal = item;
  }

  closeModal() {
    this.isModalOpen = false;
    this.articleModal = undefined;
  }
